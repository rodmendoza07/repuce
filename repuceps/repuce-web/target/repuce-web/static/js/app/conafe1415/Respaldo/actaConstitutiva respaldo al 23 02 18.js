   ...
   *****************************************/
   function _promotores(){		
	   if(!registry.byId("promotorPane")){
		   ...
		   var promotor = !actaConstitutivaObj.promotor?[]:actaConstitutivaObj.promotor;
		   var apecActa = !actaConstitutivaObj.apec?0:actaConstitutivaObj.apec.cApec;
		   ...
		   registry.byId('pestanias').addChild(new ContentPane({
			...
			//	{ name: 'genero',								field: 'genero',			width: '5px' ,hidden:true},
				{ name: 'Edad',			       					field: 'edad',     			width: '90px' },
				{ name: '\u00DAltimo grado de estudios', 			field: 'nomNiveleduc',		width: '200px'},
				{ name: 'cNiveleduc', 							field: 'cNiveleduc',		width: '5px',hidden:true},
			//	{ name: 'Curp',									field: 'curp',				width: '150px', hidden:true},								
				{ name: 'cApec', 								field: 'cApec',				hidden:true}
			]];
	   	...
	   	*/		
				var promotores = {
					cPromotor: promotores[i].promotor.cPromotor,
					cApec: promotores[i].promotor.cApec,
					paternoPromotor:promotores[i].promotor.paternoPromotor,
					maternoPromotor: promotores[i].promotor.maternoPromotor,
					nombrePromotor: promotores[i].promotor.nombrePromotor,
				//	genero: instructores[i].instructor.genero,
					edad:promotores[i].promotor.edad,
					cNiveleduc: promotores[i].promotor.cNiveleduc,
				//	curp: instructores[i].instructor.curp,
					nomNiveleduc: promotores[i].promotor.nomNiveleduc,
				//	cCcts: cCts,
					numServ: numServ,
				//	nomGenero: nomGenero
				};
				
				promotoresEncontrados.push(promotor);
	   		}
	   		
	   		if(apecActa!=0){
	   			maxIndexInstr = utils.idMaxIdPromotor(actaConstitutivaObj.apec.cApec);
	   		}
			...
			var dataStore = new dojo.data.ObjectStore({
	   		...
	   		new Button({
			//	label : constants.TEXT_BUTTON_AGREGAR_INSTRUCTOR2014,
				label : constants.TEXT_BUTTON_AGREGAR_PROMOTOR2017, // Vblake agregar en archivo contants 
				onClick : function() {
					_detallePromotor();
				}
		    }, "addRowPromot");
	   		...
	   		new Button({
			//	label : constants.TEXT_BUTTON_EDITAR_INSTRUCTOR2014,
				label : constants.TEXT_BUTTON_EDITAR_PROMOTOR2017,
				onClick : function(){
	    	...
	    	utils.createTag('div','removeRow',promotorPane);
	   		...
	   		new Button({
			//	label : constants.TEXT_BUTTON_ELIMINAR_INSTRUCTOR2014,
				label : constants.TEXT_BUTTON_ELIMINAR_PROMOTOR2017,
				onClick : function(){ 
					var gridPromotor = registry.byId('gridPromotor');
					var items = gridPromotor.selection.getSelected();
					if(items.length!=0){
 	...
 	*/       '</td></tr>'+
 	...
 	// funcion para agregar o editar un Promotor
 	   ...
 	   function _detallePromotor(itemToEditPromotor){
 	    	   ...
 	    	   '</td></tr>'+ 
 	    	   '<tr><td align="right">'+
 	    	   '	<br/><b>*N\u00FAmero de servicio:</b></td><td><br/><div id="programaEducativo"/><br/>'+
 	    	   '</td></tr>'+
 	    /*       '<tr><td align="right">'+
 	 	       '<br/><b>*Sexo H: Hombre M:Mujer: </b></td><td><br/><div id="genero"/><br/>'+
 	 	       '</td></tr>'+
 	 ...
 	 // ternmina funcion para Promotores  
      ...
      //Funcion para obtener el catalogo de los generos
 	  ...
 	  //funcion pra obtener el catalogo de cragos
