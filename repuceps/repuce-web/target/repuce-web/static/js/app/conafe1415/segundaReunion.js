define([ "dijit/layout/ContentPane","dijit/registry","dojo/_base/array","app/util/jsUtils",
	...
	function(ContentPane,registry,array,utils,json,Button,Dialog,FilteringSelect,
		...
		function init(actividades,cApec,ReunionObj, storeCcts){
			...
			_contraloriaSocial(array.indexOf(actividades,15)!=-1);
	...
	*/			
				var layoutBullyingNew = [[{name:'columna1',field:'id',hidden:true},
				                        ...
				                        {name:'N\u00famero de casos',field:'seconcreto',width:'80px',styles:'text-align: center;'},
		...
		*/		// nueva
			...
			if(pestaniaDestino.id=="inclS"){
				...
				var layoutNecesidad = [[{name:'id',field:'id',hidden:true},
											...
											{name:'En qu\u00e9 consiste el apoyo',field:'consiste1',width:'215px',styles:'text-align: left;'},
				...
				dom.byId('inclusionPane').innerHTML='<table border="0" align="left" width= "900px" cellspacing="10">'+
										'<tr>'+
											'<td width= "450px"><span class="sub" align="left">Apoyo en beneficio de ni\u00f1os con necesidades educativas especiales:</span></td>'+
		...
		//			opciones = new Array();
					...
					var opinionesObj=segundaReunionObj.opiniones?segundaReunionObj.opiniones:[];
					var denunciasObj=segundaReunionObj.denuncias?segundaReunionObj.denuncias:[];
...
//			Creamos la tabla con los elementos que tendrá el Pop UP.
	...
	*/			
			...
			var stateStore = new Memory({
		       ...
		       //     {name:"A veces", id:"A veces"},
		            {name:"Algunas veces", id:"Algunas veces"},
		      //      {name:"Pocas veces", id:"A veces"},
		            {name:"Nunca", id:"Nunca"}  
		        ]
		    });
	...
	/*		var cmbProceso1 = new FilteringSelect({
			...
			new Button({
				...
				onClick: function(){
					if(tpoApoyo=="Federal"){
						...
						_popupBullyingN();
					}else if(tpoApoyo=="accionCSoc"){
						_popupOpiniones();
					}else if(tpoApoyo=="accionCSdq"){
						_popupDenuncias();
				    }else{
		...
		function saveSegundaReunion(cApec, storeCcts) {
				...
				for ( var i in seccionesRegistradasArray) {
					...
					if (seccionesRegistradasArray[i] == constants.SECCION_CONTRALORIA_SOCIAL_R2) {
				...
				// lista.
				if (!hayApoyos && !hayInclusion && !hayPlanTrab && !hayContraSocial) {
					utils
...
///inclusion social
				...
				for ( var i = 0; i < gridNecesidades.rowCount; i++) {
					...
					if (gridNecesidades.store.getValue(item, 'necesidad') == null
							...
							|| gridNecesidades.store.getValue(item, 'seconcreto') == null
							|| gridNecesidades.store.getValue(item, 'seconcreto') == "")
...
/// Bullying
				...
				for ( var i = 0; i < gridAcoso.rowCount; i++) {
					...
					// Genera un nuevo objeto de poblacion indigena de cada
					// renglon del grid.
					if (gridAcoso.store.getValue(item, 'seconcreto') == null
							|| gridAcoso.store.getValue(item, 'seconcreto') == ""
							|| gridAcoso.store.getValue(item, 'numveces') == null
							|| gridAcoso.store.getValue(item, 'numveces') == "" 	
							|| gridAcoso.store.getValue(item, 'consiste') == null
							|| gridAcoso.store.getValue(item, 'consiste') == "") {
					...
					var Nacoso = {
						...
						consiste : gridAcoso.store.getValue(item,'consiste'),
					};
				...
				if (hayPlanTrab) {
					...
					for ( var i = 0; i < gridTrabajo.rowCount; i++) {
						...
						if (gridTrabajo.store.getValue(item, 'apoyoRecibido') == null
								...
								|| gridTrabajo.store.getValue(item, 'accion') == null
								|| gridTrabajo.store.getValue(item, 'accion') == ""	
							) {
						...
						if(gridTrabajo.store.getValue(item,'tipoRespuesta')==2){
							
							numVecesR2=gridTrabajo.store.getValue(item,'accion');
							if(numVecesR2!=null){
								respuesta=5;	
							}
							else
