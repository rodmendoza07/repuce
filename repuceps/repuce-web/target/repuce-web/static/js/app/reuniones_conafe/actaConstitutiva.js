define([ "dijit/layout/ContentPane","dijit/registry","dojo/_base/array","app/util/jsUtils",
	         ...
	         "dojo/_base/xhr","dojox/widget/Standby","app/reuniones_conafe/reuniones_conafe",
   ...
   function _integrantes(){
	   if(!registry.byId("integrantesPane")){
		   ...
		   var layout = [[
				 ...
				 { name: 'G\u00E9nero',				field: 'genero',			width: '5px', hidden:true},
		   ...
		   for(var i in integrantes){
			   ...
			   if(integrantes[i].integrante.genero=='F'){
					nomGeneroIntegrante='Femenino';
				}else{
					nomGeneroIntegrante='Masculino';
				}
	   ...
	   if(!registry.byId("instructoresPane")){
		   ...
		   registry.byId('pestanias').addChild(new ContentPane({
			...
			var dataStore = new dojo.data.ObjectStore({
	   		...
	   		//Boton Agregar
	   		new Button({
				label : constants.TEXT_BUTTON_AGREGAR_INSTRUCTOR,
				onClick : function() {
					_detalleInstructor();
				}
		    }, "addRowInstruc");
	   		...
	   		//Boton Editar
	   		new Button({
				label : constants.TEXT_BUTTON_EDITAR_INSTRUCTOR,
				onClick : function(){
	    	...
	    	utils.createTag('div','removeRow',instructoresPane);
	   		...
	   		new Button({
				label : constants.TEXT_BUTTON_ELIMINAR_INSTRUCTOR,
				onClick : function(){ 
					var gridInstructor = registry.byId('gridInstructor');
					var items = gridInstructor.selection.getSelected();
					if(items.length!=0){
   ...
   *****************************************/
			...
			//   registry.byId('pestanias').selectChild( registry.byId(asistentesPane), true);
			   ...
			   for(var i in asistentes){
				   if(asistentes[i].genero=='F'){
					   nomGeneroAsistente='Femenino';
				   }else{
					   nomGeneroAsistente='Masculino';
				   }
   ...
   function _detalleInstructor(itemToEditInstructor){
           ...
           var dDetail =new Dialog({id:'dDetail', title:'Instructor de la APEC' ,content :'<div id="dcDetail"/>'});
 	   	   dDetail.show();
 	   	   dDetail.on('hide',function(){
 	   	        registry.byId('dDetail').destroyRecursive(false);
 	   	   });
	...
	function saveActaConstitutiva(cApec, storeCcts) {
		...
		if ( registry.byId('tpoRegistro').get('value') == 0){
			utils.cstmAlert("Debe seleccionar el tipo de operaci\u00F3n");
			return false;
			} 
		...
		if (Date.parse('01/01/2011 ' + registry.byId('horaInicio').get('value')) > Date.parse('01/01/2011 ' + registry.byId('horaFinal').get('value'))){
		...
		if(gridInstructor.rowCount==0){
			utils.cstmAlert("Debe registrar al menos un instructor.");
			return false;
		}
