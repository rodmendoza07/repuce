define(["dijit/registry","dojo/_base/array","app/util/jsUtils","dojo/store/Memory",
	...
	function(registry,array,utils,Memory,dom,ValidationTextBox,FilteringSelect,constants,
			Button,xhr,Standby,Dialog,CheckedMultiSelect,CheckBox,json,reuniones,ItemFileWriteStore){
		...
		function init(actividades,cApec,ReunionObj, storeCcts){
			...
			_numeroDeAlumnos(array.indexOf(actividades,13)!=-1); // vblake Actualizar el 13 de acuerdo al numero asignado
			...
			// VBlake store para numero de alumnos
			
			xhr.get({
				url: dojo.config.app.urlBase+'catalogos/listaNumeroDeAlumnos/'+ 1+'/'+idReunion,
				sync: false, 
				preventCache:true,
				handleAs: "json",
				contentType: "application/x-www-form-urlencoded; charset=utf-8"
				...
				AlumnosStore = new Memory({data:store});
		    });
			
			// termina store para numero de alumnos 
			
	    }
		
		// Vblake Inicia Funcion Numero de Alumnos
		
		function _alumnos(crearAlumnos){
			var listPanelAlumnos= new Array({title:"Número de Alumnos", tpoList:1,id:"numeroAlum"});
			...
			for(var i in listPanelAlumnos){
				if(crearAlumnos){
					if(!registry.byId(listPanelAlumnos[i].id)){
						var objSelect = null;
						if(listPanelAlumnos[i].id=="numeroAlum"){
							objSelect = primeraReunionObj.numeroAlumnos?primeraReunionObj.numeroAlumnos:[];
						}
						_crearListaAlumnos(listPanelAlumnos[i],objSelect);
						utils.pestaniaSelect(listPanelAlumnos[0].id);
					}
				}else{
					utils.cerrarPestania(listPanelAlumnos[i].id);
				}
			}
		}
		
		
		/// termina Numero de Alumnos
		...
		function _contraloriaSocial(crearConstraloriaS){
			...
			if(crearConstraloriaS){
				if(!registry.byId(id)){
					opciones = new Array();
					var acciones=new Array();
					var accionesObj=primeraReunionObj.contraloriaSocial?primeraReunionObj.contraloriaSocial:[];
					//Creamos el layout con el nombre de las columnas de la tabla y sus propiedades.
					var layoutOpiniones = [[{name:'columna1',field:'id',hidden:true},
					                        {name:'Describir brevemente',field:'opiniones',width:'450px',styles:'text-align: center;'}]];
					var layoutDenuncias = [[{name:'columna1',field:'id',hidden:true},
					                        {name:'Describir brevemente',field:'denuncias',width:'450px',styles:'text-align: center;'}]];
					//Se crea un bloque para agregar la estructura de la tabla.
					...
					//En esta parte se crean los grid con la información
					utils.crearGrid('gridCSoc',layoutOpiniones,'id',acciones,idGridOC);
					utils.crearGrid('gridCSdq',layoutDenuncias,'id',acciones,idGridDQ);
					//Sección para los botones.
					_agregarFilaGrid('accionCSoc','buttonsAcc1');
					_agregarFilaGrid('accionCSdq','buttonsAcc2');

					utils.createTag('input','editAccionOC','buttonsAcc1');
					new Button({
						label:'Editar',
						onClick:function(){
					...
					utils.eliminarFilaGrid('gridAccionOC','buttonsAcc1',0,false);
		...
		// Vblake Inicia Numero de alumnos funcion 2
		
		function _numeroDeAlumnos(crearNumeroDeAlumnos){
			var id = "numAlum";
			var idGrid = "gridNumAlum";
		//	var idGridDQ = "gridAccionDQ";
			if(crearNumeroDeAlumnos){
				if(!registry.byId(id)){
					opciones = new Array();
					var alumnos=new Array();
					var alumnosObj=primeraReunionObj.crearnumeroDeAlumnos?primeraReunionObj.crearnumeroDeAlumnos:[];
					//Creamos el layout con el nombre de las columnas de la tabla y sus propiedades.
					var layoutAlumnos = [[{name:'columna1',field:'id',hidden:true},
					                      	{name:'contador1',field:'contador1',hidden:true},
				    						{name:'contador2',field:'contador2',hidden:true},
					                      	{name:'Estado',field:'estado',width:'450px',styles:'text-align: center;'},
											{name:'Municipio',field:'municipio',width:'450px',styles:'text-align: center;'},
											{name:'Localidad',field:'localidad',width:'450px',styles:'text-align: center;'},
											{name:'Clave de Centro de Trabajo',field:'opiniones',width:'30px',styles:'text-align: center;'},
											{name:'Número de niños',field:'opiniones',width:'20px',styles:'text-align: center;'},
											{name:'Numero de niñas',field:'opiniones',width:'20px',styles:'text-align: center;'}]];
									
					 
					//Se crea un bloque para agregar la estructura de la tabla.
					utils.crearPanel("numAlum","Número de Alumnos",'numeroDeAlumnosPane');
					dom.byId('numeroDeAlumnosPane').innerHTML='<table border="0" width= "900px" cellspacing="10">'+
												'<tr>'+
													'<td><span align="left" class="sub" align="left">Número de Alumnos</span></td>'+
												
												'</tr>'+
												'<tr>'+
													//'<td width= "450px" valign="top"><input id="cmsNumeroA"/></td>'+
					...
					//En esta parte se crean los grid con la información
					utils.crearGrid('gridNumAlum',layoutAlumnos,'id',alumnos,idGrid);
					...
					utils.createTag('input','editAccionOC','buttonsAcc1');
					new Button({
						label:'Editar', // avance numero de alumnos vblake
						onClick:function(){
							var gridOC = registry.byId(idGridOC);
							var items = gridOC.selection.getSelected();
							if(items.length==1){
		...
		// Vblake termina Numero de alumnos funcion 2
	...
	/*		var cmbGestion = new FilteringSelect({
		...
		function _popupOpiniones(itemToEdit){
			...
			var dDetail =new Dialog({
					id:idVentana, 
					title:"Acci\u00F3n programada  por vblake opiniones", 
					content:'<table border="0" width= "900px">'+
							/*	'<tr>'+
									'<td align="right"><label>* Apoyo: </label></td>'+
									'<td><input id="cmbApoyo"/></td>'+
								...
								'</tr>'+ */
								'<tr>'+
									'<td align="right"><label>* \u00BFOpiniones y comentarios adicionales ventana: </label></td>'+
		...
		function _popupDenuncias(itemToEdit){
			...
			var dDetail =new Dialog({
					id:idVentana, 
					title:"Acci\u00F3n programada  por vblake denuncias", 
					content:'<table border="0" width= "900px">'+
								'<tr>'+
									'<td align="right"><label>* \u00BFDenuncias o quejas ventana: </label></td>'+
			...
			function savePrimeraReunion(cApec, storeCcts) {
				...
				if(!hayApoyos && !hayDiagComunitario && !hayPobInd && !hayPlanTrab && !hayContraSocial){
...
1
				...
				var denunciasArreglo = new Array();
				var opinionesArreglo = new Array();
				...
				if (hayContraSocial) {
					...
					for ( var i = 0; i < gridCSoc.rowCount; i++) {
						...
						opinionesArreglo.push(accionCSoc);
						hayContraSocial = true;
					}
					...
					for ( var i = 0; i < gridCSdq.rowCount; i++) {
						...
						denunciasArreglo.push(accionCSdq);
						hayContraSocial = true;
					}
				...
				var primeraReunion = {
					...
					opiniones : opinionesArreglo,
					denuncias: denunciasArreglo,
