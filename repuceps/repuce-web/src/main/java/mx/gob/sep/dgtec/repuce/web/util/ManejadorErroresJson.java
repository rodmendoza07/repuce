...
import mx.gob.sep.dgtec.repuce.servicios.util.BaseTipoError;
...
/**
 * 
 *  Una extension de {@link AbstractHandlerExceptionResolver} con las siguientes caracteristicas:
 *  - Devuelve informacion del error en formato Json.
 *  - Se puede asignar un mecanismo de notificacion a cada tipo de excepcion configurada.
 *  - Se puede asignar una etiqueta arbitraria para indicar el nivel de gravedad de cada excepcion.
 *
 *  El siguiente es un ejemplo de configuracion tipica.
 * <pre>
 * {@code
 *        <bean class="mx.gob.sep.dgtec.repuce.web.util.ManejadorErroresJson">
 *            <property name="mapeoExcepciones">
 *                <map>
 *                    <!-- Para asignar un notificador a una Excepcion. Si no se establece un notificador
 *                    para una excepcion mas especifica se elige el notificador de la super clase mas cercana
 *                    en la jerarquia de herencia.-->
 *                    <entry key="mx.gob.sep.dgtec.repuce.web.util.util.ErrorNegocio" value-ref="notificacionLog"></entry>
 *
 *                    <!-- En este caso no se elige notificador. Si se especifica un servicioNotificacionDefault
 *                    se usara este, de otro modo no se notificara el error. El error hacia presentacion siempre
 *                    se genera.-->
 *                    <entry key="mx.gob.sep.dgtec.repuce.web.util.ErrorInfraestructura" value=""></entry>
 *    
 ...
 *                    <entry key="mx.gob.sep.dgtec.repuce.web.util.ErrorNegocio" value="ERROR DE NEGOCIO"></entry>
 *                    <entry key="mx.gob.sep.dgtec.repuce.web.util.ErrorInfraestructura" value="ERROR GRAVE"></entry>
 ...
 *        </bean>
 *       <bean id="notificacionLog" class="mx.gob.sep.dgtec.repuce.web.util.ServicioNotificacionLog" />
 * }
 * </pre>
 *
...
public class ManejadorErroresJson
    ...
    /**
     * Resuelve una excepcion generando un objeto con informacion del error,
     * en formato Json.
     *
     * El objeto contiene por defecto todos los campos de la clase {@link InformacionError},
     * excepto el campo excepcion, esto reduce la informacion enviada al cliente.
     *
     * Posteriormente el metodo usa el objeto de notificacion de error para enviar la
     * informacion del mismo a algún medio como: correo, archivo, syslog, etc.
     *
     * @param request
     * @param response
     * @param handler El objeto del cual provino la excepcion. Por ejemplo un controller.
     * @param ex
    ...
    protected ModelAndView doResolveException(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
        if(log.isDebugEnabled()) {
            log.debug("Resolviendo mensaje JSON para excepcion:" + ex.getClass().
                    getCanonicalName());
        }
        ...
        aplicarCodigoErrorHttp(request, response, codigoErrorHttpDefualt);
    ...
    /**
     * Busca la clase de excepcion, registrada en el mapeo, mas cercana en
     * cuanto a herencia, para asi determinar el servicio de notificacion para
     * la excepcion.
     *
    ...
    /**
     * Busca la clase de excepcion, registrada en el mapeo, mas cercana en
     * cuanto a herencia, para asi determinar la etiqueta de nivel del gravedad del error.
     *
     * @param exLanzada
     * @return
     */
    ...
    /**
     * Calcula la distancia en la jerarquia de herencia entre la excepcion lanzada y
     * una de las excepciones mapeadas en el bean.
     *
     * @param exLanzada
     * @param exMapeada
     * @return 0 si son del mismo tipo. -1 Si exLanzada y exMapeada no tienen relacion de herencia.
     */
    ...
    /**
     * Calcula la distancia en la jerarquia de herencia entre la excepcion lanzada y
     * una de las excepciones mapeadas en el bean. El metodo usa recursividad.
     *
     * @param exLanzada
     * @param exMapeada
     * @param distancia
     ...
     * Usa ademas el metodo prepareResponse de la super clase {@link AbstractHandlerExceptionResolver}
     * para configurar aspectos del cache. Por ejemplo, se desactiva el cache.
     *
    ...
    protected void prepareResponse(Exception ex, HttpServletResponse response) {
        if(log.isDebugEnabled()) {
            log.debug("Preparando objeto reponse para informacion de error.");
        }
    ...
    /**
     * Prepara un objeto con toda la informacion del error que se ha generado.
     *
    ...
    protected InformacionError prepararInformacionError(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
        ...
        String mensaje = messageSource.getMessage("general.mensajeError.default", new Object[]{},
                                                  "Ocurrio un error inesperado. Consulte con el administrador usando la clave del error.", LocaleUtil.getLocale());
...
//                    mensaje = messageSource.getMessage(eb.getCodes()[0], eb.getArguments(), SeguridadUtil.getLocale());
                    ...
                    logger.error("Error al resolver el mensaje i18n para la excepcion (se usara mensaje generico): " + ex.
                            getMessage(), e);
                }
            }
    ...
    /**
     * Prepara informacion del error dentro de un {@link BeanMap}.
     *
     * @param request
     * @param response
     * @param handler
     * @param ex
     * @return Un objeto de tipo BeanMap con toda la informacion del error.
     */
    ...
    /**
     * Devuelve un Map con la informacion del error quitando las entradas de
     * las llaves mencionadas en el arreglo filtradas.
     *
     * Esto ayuda a reducir la informacion enviada al cliente. Los campos que se
     * pueden filtrar al momento son los de la clase {@link InformacionError}.
     *
     * @param informacion Mapa con la informacion del error.
     * @return Un objeto de tipo BeanMap con toda la informacion del error.
     */
     ...
     * el campo codigoErrorHttpDefualt.
     *
     * @param request
     * @param response
     * @param statusCode
    ...
    protected void aplicarCodigoErrorHttp(HttpServletRequest request, HttpServletResponse response, int statusCode) {
        if(!WebUtils.isIncludeRequest(request)) {
            if(log.isDebugEnabled()) {
                log.debug("Aplicando codigo de error HTTP " + statusCode);
            }
     ...
     */
    public void setCodigoErrorHttpDefualt(Integer codigoErrorHttpDefualt) {
        this.codigoErrorHttpDefualt = codigoErrorHttpDefualt;
    }
